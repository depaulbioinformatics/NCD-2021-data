...
# create a CSV for easier (? hopefully, I guess?) data validation
averages = open('Avg_Timing_Spreadsheet.csv','w')
averages.write('Method, 0, 1, 2, 3, 4, 5\n')

for key in d:
    line = key
    for subKey in d[key]:
        avg = round((sum(d[key][subKey])/len(d[key][subKey]))*60,14)
        line = line + "," + str(avg)
    line = line + '\n'
    averages.write(line)

averages.close()
print("done... wrote averages and report to csv")
...
##    for subKey in d[key]:
##        print(f'{round((sum(d[key][subKey])/len(d[key][subKey]))*60,14):15}', end = '\t')
##        avg = round((sum(d[key][subKey])/len(d[key][subKey]))*60,14)
##    print()
